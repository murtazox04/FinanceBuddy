...
async def expense_main_category_keyboard(menu, _id, user_id, parent_id):
    builder = InlineKeyboardBuilder()
    main_categories = await get_categories(_id=_id, user_id=user_id, parent_id=parent_id)
    if not main_categories:
        return None
    for category in main_categories:
        category_name = category.name if len(category.name) <= 30 else category.name[:27] + "..."
        builder.button(
            text=category_name,
            callback_data=ExpenseCallback(category_type=menu, category_id=category.pk).pack()
        )
    builder.button(
...
def expense_search_keyboard(search_text):
    builder = InlineKeyboardBuilder()
    builder.button(
        text=f"🔍 {search_text}",
